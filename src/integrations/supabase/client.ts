...
(async () => {
  try {
    ...
    const timeoutPromise = new Promise<{ data: { session: null }, error: AuthError }>((_, reject) => {
      setTimeout(() => reject(new Error("Timeout lors de la récupération de la session")), 20000); // Timeout augmenté à 20 secondes
    });
    ...
    try {
      ...
      const testResult = await Promise.race([
        ...
        new Promise<{ error: AuthError }>((_, reject) => 
          setTimeout(() => reject(new Error("Timeout de connexion à la base")), 20000) // Timeout augmenté à 20 secondes
        )
      ]);
