...
export function useRequestAssignment(onRequestUpdated: () => void) {
  const assignRequestToMe = async (requestId: string) => {
    try {
      ...
      // Notify the SDR - using proper column specification
      ...
      if (request && request.created_by_profile && request.created_by_profile.email) {
  ...
  const updateRequestWorkflowStatus = async (requestId: string, newStatus: string) => {
    try {
      ...
      // Notify the SDR about the status change - using proper column specification
      const { data: request } = await supabase
        .from('requests')
        .select(`
          *,
