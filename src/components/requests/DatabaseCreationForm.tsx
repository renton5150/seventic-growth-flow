...
export const DatabaseCreationForm = ({ editMode = false, initialData, onSuccess }: DatabaseCreationFormProps) => {
  ...
  useEffect(() => {
    const initializeBuckets = async () => {
      ...
      try {
        ...
        if (success) {
          console.log("DatabaseCreationForm - Buckets vérifiés avec succès");
          setBucketsReady(true);
        } else {
          console.warn("DatabaseCreationForm - Problème avec les buckets mais continuons");
          setInitializationWarning("Certaines fonctionnalités de stockage peuvent être limitées");
          setBucketsReady(true); // Permettre au formulaire de se charger quand même
        }
      } catch (error) {
        console.error("DatabaseCreationForm - Erreur initialisation buckets:", error);
        setInitializationWarning("Problème de connexion au stockage, mais le formulaire reste utilisable");
        setBucketsReady(true); // Toujours permettre au formulaire de se charger
      }
    };
  ...
  const handleFileUpload = async (field: string, files: FileList | null | string) => {
    ...
    // Vérification de buckets moins stricte
    if (!bucketsReady) {
      console.warn("DatabaseCreationForm - Les buckets ne sont pas complètement vérifiés, mais continuons");
      toast.warning("Le système de stockage n'est pas complètement vérifié, mais nous allons essayer.");
    }
  ...
  // Afficher un avertissement s'il y en a un, mais permettre au formulaire de se charger
  if (initializationWarning) {
    toast.warning(initializationWarning);
    setInitializationWarning(null); // Ne l'afficher qu'une fois
  }

  // Afficher un indicateur de chargement seulement pendant la vérification initiale
  if (!isFormReady) {
    return (
      ...
      {initializationWarning && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
          <div className="flex items-center">
            ...
            <div className="text-yellow-800">{initializationWarning}</div>
          </div>
        </div>
      )}
